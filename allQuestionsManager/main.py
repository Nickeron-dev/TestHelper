...
def main():
    ...
    try:
        ...
        while True:
            ...
            text_illia = service.files().get_media(fileId=file_id_illia).execute().decode('utf-8')
            file_id_daria = '1if3GwNgPMyod72awNrHyHrMi3mZxfij2'
            text_daria = service.files().get_media(fileId=file_id_daria).execute().decode('utf-8')
                ...
                if content_new[i] != '\n':
                    ...
                    for j in range(i + 1, len(content_new)):
                        if content_new[j] != '\n':
                            ...
                            if line_i == line_j and line_i != '' and (not repeated_questions.keys().__contains__(line_i) or repeated_questions.get(line_i) < 2):
                                count += 1
